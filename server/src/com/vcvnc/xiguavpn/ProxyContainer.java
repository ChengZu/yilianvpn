...
public class ProxyContainer {
	...
	public void processPacket(byte[] packet, int size, char protocol) {
		...
		if (proxys.size() > Config.CLIENT_MAX_PROXY) {
			...
			if (clearNum > 0)
				System.out.printf("[%s][Proxycontainer]client(%d) tcpProxys size: %d max, clean proxy num: %d.\n",
		...
		if (proxy == null) {
			if (protocol == IPHeader.TCP) {
				...
				System.out.printf("[%s][Proxycontainer]client(%d) create new unkown proxy(%s) error.\n",
