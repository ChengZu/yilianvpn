...
public class ProxyContainer {
	...
	// 代理数组
	public List<Proxy> proxys = Collections.synchronizedList(new ArrayList<Proxy>());
	...
	public void processPacket(byte[] packet, int size, char protocol) {
		...
		if (proxys.size() > Config.CLIENT_MAX_PROXY) {
			...
			if (clearNum > 0)
				System.out.printf("[%s][Proxycontainer]Client(%d) have clear proxy num: %d, now size: %d.\n",
						CommonMethods.formatTime(), client.id, proxys.size(), clearNum);
		}
		...
		if (proxy == null) {
			if (protocol == IPHeader.TCP) {
				...
				System.out.printf("[%s][Proxycontainer]Client(%d) create new unknown proxy(%s) error.\n",
						CommonMethods.formatTime(), client.id, proxy);
				return;
			}
			proxys.add(proxy);
