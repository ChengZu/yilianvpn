...
public class TcpProxy extends Proxy implements Runnable {
	...
	public ArrayList<byte[]> clientBuffer = new ArrayList<byte[]>();
	...
	private void connect() {
		try {
			...
			for (int i = 0; i < clientBuffer.size(); i++) {
				try {
					os.write(clientBuffer.get(i), 0, clientBuffer.get(i).length);
				} catch (IOException e) {
					close();
					break;
				}
	...
	public void processFisrtPacket(byte[] packet, int size) {
		...
		if ((flags & TCPHeader.SYN) == TCPHeader.SYN) {
			...
			// System.out.printf("[TcpProxy]proxy(%s) recvive client first packet(%s) is not
	...
	public void processPacket(byte[] packet, int size) {
		...
		if ((flags & TCPHeader.SYN) == TCPHeader.SYN) {
			...
			System.out.printf("[%s]Client(%d) tcpProxy(%s:%d) 未处理标志位%d{%s}.\n", CommonMethods.formatTime(), client.id,
					CommonMethods.ipIntToString((int) ipHeader.getDestinationIP()), tcpHeader.getDestinationPort(),
					flags, tcpHeader.toString());
		}
	}
	...
	public void processACKPacket(byte[] packet) {
		...
		case CLOSED:
			...
			break;
		default:
			System.out.printf("[%s]TcpProxy(%d) No function deal ACK packet.\n", CommonMethods.formatTime(), id);
			break;
		}
	}
	...
	public void processSYNRCVDACKPacket(byte[] packet) {
		...
		if (tcpHeader.getSeqID() == myAck && tcpHeader.getAckID() == mySeq) {
			...
			System.out.printf("[%s]TcpProxy(%d) Bad SYNRCVDACK.\n", CommonMethods.formatTime(), id);
		}
	}
	...
	public void processESTABLISHEDACKPacket(byte[] packet) {
		...
		if (seq == myAck) {
			...
			if (nextSeq > myAck) {
				System.out.printf("[%s]TcpProxy(%d) ACK seq %d max %d, more packets.\n", CommonMethods.formatTime(), id,
						nextSeq, myAck);
	...
	public void processLASTACKPacket(byte[] packet) {
		...
		if (ack == mySeq && seq == myAck) {
			close();
			// System.out.printf("TcpProxy(%d) LAST_ACK succeed, closed, seq %d:%d, ack
			// %d:%d.\n", id, seq %d:%d, ack %d:%d.\n", id, seq, myAck, ack, mySeq);
		} else {
			System.out.printf("[%s]TcpProxy(%d) LAST_ACK failed, queue number mismatched, seq %d:%d, ack %d:%d.\n",
					CommonMethods.formatTime(), id, seq, myAck, ack, mySeq);
		}
	}
	...
	public boolean equal(byte[] packet) {
		...
		return srcIp == ipHeader.getSourceIP() && srcPort == tcpHeader.getSourcePort()
				&& destIp == ipHeader.getDestinationIP() && destPort == tcpHeader.getDestinationPort();
	}
