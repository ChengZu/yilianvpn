...
public class TcpProxy extends Proxy implements Runnable {
	...
	private final int MY_CLOSE_WAIT = 5;
	private final int MY_LAST_ACK = 6;
	private final int CLOSED = 7;
	// 与目标服务器的套接字
	...
	public void processFisrtPacket(byte[] packet, int size) {
		...
		if ((flags & TCPHeader.SYN) == TCPHeader.SYN) {
			...
			errorClose();
			//System.out.printf("[%s][TcpProxy](%s) recvive client first packet(%s) is not syn, flags is %d.\n", CommonMethods.formatTime(), toString(), tcpHeader.toString(), flags);
		}
	}
	...
	public void processACKPacket(byte[] packet) {
		...
		case LAST_ACK:
			...
			break;
		case MY_CLOSE_WAIT:
			processMYCLOSEWAITACKPacket(packet);
			break;
		case MY_LAST_ACK:
			processMYLASTACKPacket(packet);
			break;
		case CLOSED:
	...
	public void processSYNWAITACKPacket(byte[] packet) {
		if (tcpHeader.getSeqID() == clientSeq && tcpHeader.getAckID() == serverSeq) {
			...
			errorClose();
			System.out.printf("[%s][TcpProxy](%s) SYN_WAIT_ACK error, packet(%s).\n", CommonMethods.formatTime(), toString(), tcpHeader.toString());
		}
	}
	...
	public void processMYCLOSEWAITACKPacket(byte[] packet) {
		int ack = tcpHeader.getAckID();
		int seq = tcpHeader.getSeqID();
		int flags = tcpHeader.getFlag();
		if (flags == (TCPHeader.FIN | TCPHeader.ACK) && ack == (serverSeq + 1) && seq == clientSeq) {
			state = MY_LAST_ACK;
			//System.out.printf("[%s]TcpProxy(%d) MY_CLOSE_WAIT_ACK succeed, seq(%d:%d),ack(%d:%d).\n", CommonMethods.formatTime(), id, seq, clientSeq, ack, (serverSeq + 1));
		} else {
			//System.out.printf("[%s]TcpProxy(%d) MY_CLOSE_WAIT_ACK failed, seq(%d:%d), ack(%d:%d).\n", CommonMethods.formatTime(), id, seq, clientSeq, ack, (serverSeq + 1));
		}
		
	}

	public void processMYLASTACKPacket(byte[] packet) {
		int ack = tcpHeader.getAckID();
		int seq = tcpHeader.getSeqID();
		int flags = tcpHeader.getFlag();
		if (flags == (TCPHeader.FIN | TCPHeader.ACK) && ack == (serverSeq + 1) && seq == (clientSeq + 1)) {
			updateTCPBuffer(headerBytes, (byte) (TCPHeader.FIN | TCPHeader.ACK), (serverSeq + 1), (clientSeq + 2), 0);
			sendToClient(headerBytes, TcpProxy.HEADER_SIZE);
			close();
			System.out.printf("[%s]TcpProxy(%d) MY_LAST_ACK succeed, seq(%d:%d),ack(%d:%d).\n", CommonMethods.formatTime(), id, seq, (clientSeq + 1), ack, (serverSeq + 1));
		} else {
			System.out.printf("[%s]TcpProxy(%d) MY_LAST_ACK failed, seq(%d:%d), ack(%d:%d).\n", CommonMethods.formatTime(), id, seq, (clientSeq + 1), ack, (serverSeq + 1));
		}
	}
