...
public class UdpProxy extends Proxy implements Runnable {
	...
	public void processPacket(byte[] packet, int size) {
		...
		int headerSize = ipHeader.getHeaderLength() + UDPHeader.UDP_HEADER_SIZE;
		int dataSize = ipHeader.getTotalLength() - headerSize;

		byte[] data = new byte[dataSize];
		System.arraycopy(packet, headerSize, data, 0, dataSize);
		...
		while (size != -1 && !closed && noError) {
			try {
				...
				if (size > 0) {
					...
					System.arraycopy(revPacket.getData(), 0, packet, HEADER_SIZE, size);
