...
public class Client implements Runnable {
	...
	private int processRecvPacket(byte[] bytes, int size) {
		...
		if(totalLength < 0 || totalLength > Config.MUTE) { //长度非法 
			System.out.printf("Client(%d) recvive bad length packet.\n", id);
			return -2;
		}
	...
	private int processIPPacket(byte[] bytes, int size) {
		...
		if (protocol == IPHeader.TCP) {
			...
			System.out.printf("Client(%d) recvive unknown protocol packet.\n", id);
			ret = -1;;
		}
		return ret;
	}
	...
	public void run() {
		...
		int size = 0;
		// 读取头Config.USER_INFO_HEADER_SIZE字节，验证用户名密码
		int readSize = 0;
		byte[] readBytes = new byte[Config.USER_INFO_HEADER_SIZE];
		while (size != -1 && readSize < Config.USER_INFO_HEADER_SIZE && noError) {
			try {
				size = is.read(readBytes, readSize, Config.USER_INFO_HEADER_SIZE - readSize);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				noError = false;
