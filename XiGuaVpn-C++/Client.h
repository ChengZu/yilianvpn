	...
	bool loop(){
		...
		if(confirmUser){
			...
			if (size > 0) {
				...
				lastRefreshTime = std::time(NULL);
				if(res == -1) {
					printf("Client(%ld) info: Recived damaged packet, closeing.\n", myId);
					close();
					ret = true;
				}else if(res == -2){
					printf("Client(%ld) info: Recived unkown protocol packet, closeing.\n", myId);
					close();
					ret = true;
				}
			}else if(size == 0){
				printf("Client(%ld) info: Remote connection disconnected, closeing.\n", myId);
				close();
				ret = true;
			}
			delete[] bytes;
			...
			if (header.getSourceIP() != Config::USER_NAME && header.getDestinationIP() != Config::USER_PASSWD) {
				printf("Client(%ld) info: Incorrect username or password, closeing.\n", myId);
