	...
	bool loop(){
		...
		if(confirmUser){
			...
			if (size > 0) {
				...
				lastRefreshTime = std::time(NULL);
				if(res == -1) {
					printf("[Client]客户端(%ld)收到坏的TCP/IP包，正在进行关闭。\n", myId);
					close();
					ret = true;
				}else if(res == -2){
					printf("[Client]客户端(%ld)收到未知协议的TCP/IP包，正在进行关闭。\n", myId);
					close();
					ret = true;
				}
			}else if(size == 0){
				printf("[Client]客户端(%ld)失去连接，正在进行关闭。\n", myId);
				close();
				ret = true;
			}
			delete[] bytes;
			...
			if (header.getSourceIP() != Config::USER_NAME && header.getDestinationIP() != Config::USER_PASSWD) {
				printf("[Client]客户端(%ld)建立连接校验接用户信息失败，正在进行关闭。\n", myId);
				socket->socketSend((char*)"Access denied", 13);
				close();
				return true;
			}
			confirmUser = true;
			printf("[Client]客户端(%ld)用户%ld信息校验成功，建立连接成功。\n", myId, header.getSourceIP());
		}
		return ret;
	}
