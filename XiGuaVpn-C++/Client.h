...
public:
    ...
    Client(Socket *socket)
    {
        ...
        cacheBytes = NULL;
        cacheBytesSize = 0;
        proxyContainer = ProxyContainer(clientId, socket);
        lastRefreshTime = std::time(NULL);
        buffer = new char[Config::MUTE];
    ...
    int processIPPacket(char *packet, int size)
    {
        ...
        {
            proxyContainer.processPacket(packet, size, IPHeader::TCP);
        }
        else if(protocol == IPHeader::UDP)
        {
            proxyContainer.processPacket(packet, size, IPHeader::UDP);
        }
        else
        {
            return -2; //无法处理的IP协议
    ...
    int processRecvBytes(char *bytes, int size)
    {
        ...
        if (this->cacheBytesSize > 0)
        {
            ...
            ret = processRecvBytes(data, size);
            delete[] data;
            return ret;
        }
