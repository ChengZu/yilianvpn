...
public:
	VpnServer(){
		...
		if(res == -1){
			perror("error msg");
			printf("VpnServer error: bind 0.0.0.0:%d unsusscessful.\n", Config::PORT);
			close();
		}else{
			printf("VpnServer bind 0.0.0.0:%d.\n", Config::PORT);
	...
	bool loop(){
		...
		if(socket_fd == -1){
			...
			printf("VpnServer error: socket closed.\n");
			close();
			return true;
		}
		...
		if (clients.size() < Config::MAX_CLIENT_NUM) {
			...
			printf("New client(%ld) connect, total client %ld.\n", client->myId, clients.size());
		} else {
			socket->iClose();
			printf("Client connect max(%ld), connect closed.\n", clients.size());
		}
		return false;
	}
