...
public:	
	...
	UdpProxy(long clientId, Socket* clientSocket, char* packet) : Proxy(clientId, clientSocket, packet){
		...
		UDPHeader udpHeader = UDPHeader(muteBuffer, IPHeader::IP4_HEADER_SIZE);
	...
	int sendToServer(char* bytes, int size){
		...
		int res = sendToBuffer(&destSocket, &clientRingQueueBuffer, bytes, size);
		if(res < size){
	...
	bool loop(){
		...
		// 从服务器接收数据  
		int size = destSocket.socketRecv(muteBuffer + Proxy::UDP_HEADER_SIZE, Config::MUTE - Proxy::UDP_HEADER_SIZE);
		if (size > 0) {
			updateUDPBuffer(muteBuffer, size);
			// 转发给客户端 
			sendToClient(muteBuffer, Proxy::UDP_HEADER_SIZE + size);
		}else if(size == 0){
			close();
			return true;
		}
