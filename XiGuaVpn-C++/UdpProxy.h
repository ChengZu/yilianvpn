...
public:
	...
	int sendToServer(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(res == 0){
			...
			printf("[UdpProxy]proxy(%ld%s) forward data to server fail, retry=%d.\n", id, toString().c_str(), retry);
			if(retry > 0){
		...
		if(retry == 0 && (offset + dataSize) != size){
			close();
			perror("[UdpProxy]socket error msg");
			printf("[UdpProxy]proxy(%ld%s) forward data to server fail, total %d bytes, success send %d bytes.\n", id, toString().c_str(), size, (offset + dataSize));
		}
		return res;
	}
	...
	int sendToClient(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(res == 0){
			...
			perror("[UdpProxy]socket error msg");
			printf("[UdpProxy]proxy(%ld%s) forward data to client fail, retry=%d.\n", id, toString().c_str(), retry);
			if(retry > 0){
		...
		if(retry == 0 && (offset + dataSize) != size){
			close();
			perror("[UdpProxy]socket error msg");
			printf("[UdpProxy]proxy(%ld%s) forward data to client fail, total %d bytes, success send %d bytes.\n", id, toString().c_str(), size, (offset + dataSize));
		}
		return res;
	}
