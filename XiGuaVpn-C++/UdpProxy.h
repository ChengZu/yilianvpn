...
public:
	...
	int sendToServer(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(retry == 0 && (offset + dataSize) != size){
			close();
			perror("error msg");
			printf("UdpProxy(%ld) error: sendToServer packet min %d @ %d.\n", id, (offset + dataSize), size);
		}
		return res;
	}
	...
	int sendToClient(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(res == 0){
			...
			//perror("error msg");
			//printf("UdpProxy(%ld) error: sendToClient retry=%d.\n", id, retry);
			if(retry > 0){
		...
		if(retry == 0 && (offset + dataSize) != size){
			close();
			perror("error msg");
			printf("UdpProxy(%ld) sendToClient error min %d @ %d.\n", id, (offset + dataSize), size);
		}
		return res;
	}
