...
public:
	...
	int sendToServer(char* bytes, int size){
		...
		if(connected){
			...
			if(rlen > 0){
				...
				res = sendData(&destSocket, data, rlen);
				delete[] data;
				if(res < rlen) { 
					clientRingQueueBuffer.push(data + res, rlen - res);
					int notWriteSize = size -  clientRingQueueBuffer.push(bytes, size);
					if(notWriteSize > 0){
						printf("ERROR:[Proxy]proxy(%s) clientRingQueueBuffer no buffer space %d less %d.\n", toString().c_str(), clientRingQueueBuffer.availableWriteLength(), notWriteSize);
					}
					return -1;
				}
			}
			...
			if(res < size){
				//缓存未发送的数据 
				int notWriteSize = size - res -clientRingQueueBuffer.push(bytes + res, size - res);
				if(notWriteSize > 0){
					printf("ERROR:[Proxy]proxy(%s) clientRingQueueBuffer no buffer space %d less %d.\n", toString().c_str(), clientRingQueueBuffer.availableWriteLength(), notWriteSize);
				}
			}
			...
			if(!getClientFirstChar){
				if(size > 0){
					...
					notWriteSize = size - 1 - len;
				}
			}else{
				int len = clientRingQueueBuffer.push(bytes, size);
				notWriteSize = size - len;
			}
			if(notWriteSize > 0) {
				printf("ERROR:[Proxy]proxy(%s) clientRingQueueBuffer no buffer space %d less %d.\n", toString().c_str(), clientRingQueueBuffer.availableWriteLength(), notWriteSize);
			}
	...
	int sendToClient(char* bytes, int size){
		...
		int wlen = serverRingQueueBuffer.availableWriteLength();
		if(rlen > 0){
			char* data = new char[rlen];
			serverRingQueueBuffer.poll(data, rlen);
			res = sendData(&destSocket, data, rlen);
			delete[] data;
			if(res < rlen) {
				serverRingQueueBuffer.push(data + res, rlen - res);
				int notWriteSize = size - serverRingQueueBuffer.push(bytes, size);
				if(notWriteSize > 0){
					printf("ERROR:[Proxy]proxy(%s) serverRingQueueBuffer no buffer space %d less %d.\n", toString().c_str(), serverRingQueueBuffer.availableWriteLength(), notWriteSize);
				}
				return -1;
			}
		}
		
		res = sendData(clientSocket, bytes, size);
		if(res < size){
			//缓存未发送的数据 
			int notWriteSize = size - res - serverRingQueueBuffer.push(bytes + res, size - res);
			if(notWriteSize > 0){
				printf("ERROR:[Proxy]proxy(%s) serverRingQueueBuffer no buffer space %d less %d.\n", toString().c_str(), serverRingQueueBuffer.availableWriteLength(), notWriteSize);
			}
		}
	...
	void sendBuffer(){
		...
		//数据即可发送给客户端 
		int rlen2 = serverRingQueueBuffer.availableReadLength();
		if(rlen2 > 0){
			char* data = new char[rlen2];
			serverRingQueueBuffer.poll(data, rlen2);
			int res = sendData(clientSocket, data, rlen2);
			if(res < rlen2) {
				serverRingQueueBuffer.push(data + res, rlen2 - res);
			}
			delete[] data;
		}
	} 
	...
	bool loop(){
		...
		int size = destSocket.socketRecv(bytes, len);
		if (size > 0) {
...
private:
	...
	RingQueueBuffer serverRingQueueBuffer;
