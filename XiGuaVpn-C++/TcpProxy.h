...
public:
	...
	int sendToClient(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(res > 0) dataSize = res;
		if(retry == 0 && (offset + dataSize) != size){
			close("send data to client error");
			perror("[TcpProxy]socket error msg");
	...
	void processPacket(std::vector<TcpProxy*> *tcpProxys, char* packet, bool newTcpProxy) {
		...
		if(newTcpProxy && (flags & TCPHeader::SYN) != TCPHeader::SYN) {
			...
			if((flags & TCPHeader::RST) != TCPHeader::RST){
				disConnectClient("recvive client first packet is not syn");
			}else{
				errorMsg = "recvive client first packet is not syn";
				close();
			}
			return;
		}
		if(newTcpProxy) {
			// printf("[TcpProxy]proxy(%s) recvive client first packet(%s) is syn, flags is %d, created.\n", toString().c_str(), tcpHeader.toString().c_str(), flags);
	...
	void processRSTPacket(char* packet) {
		...
		errorMsg = "recvive client rst packet";
		close();
	}
	...
	void processLASTACKPacket(char* packet) {
		...
		if (seq == mySeq && ack == myAck) {
			// printf("[TcpProxy]proxy(%s) recvive LAST_ACK success, close suceess，seq %u:%u, ack %u:%u.\n", toString().c_str(), seq, mySeq, ack, myAck);
			errorMsg = "closed done";
			close();
		} else {
			printf("[TcpProxy]proxy(%s) recvive LAST_ACK fail, close fail, queue number mismatched, seq %u:%u, ack %u:%u.\n", toString().c_str(), seq, mySeq, ack, myAck);
		}
	}
