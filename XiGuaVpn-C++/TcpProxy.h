...
public:
	...
	void close(){
		...
		iloop.quit();
		destSocket.iClose();
		// printf("[TcpProxy]proxy(%ld) closed.\n", id);
	}
	...
	int sendToServer(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(connected){
			...
			if(res == 0){
				...
				printf("[TcpProxy]proxy(%ld%s) forward data to server fail, retry=%d.\n", id, toString().c_str(), retry);
				if(retry > 0){
			...
			if(retry == 0 && (offset + dataSize) != size){
				disConnectClient();
				perror("[TcpProxy]socket error msg");
				printf("[TcpProxy]proxy(%ld%s) forward data to server fail, total %d bytes, success send %d bytes.\n", id, toString().c_str(), size, (offset + dataSize));
			}
		}else{
	...
	int sendToClient(char* bytes, int size, int offset = 0, int retry = 2){
		...
		if(res == 0){
			...
			perror("[TcpProxy]socket error msg");
			printf("[TcpProxy]proxy(%ld%s) forward data to client fail, retry=%d.\n", id, toString().c_str(), retry);
			if(retry > 0){
		...
		if(retry == 0 && (offset + dataSize) != size){
			//disConnectClient();
			perror("[TcpProxy]socket error msg");
			printf("[TcpProxy]proxy(%ld%s) forward data to client fail, total %d bytes, success send %d bytes.\n", id, toString().c_str(), size, (offset + dataSize));
		}
		return res;
	}
	...
	void processPacket(std::vector<TcpProxy*> *tcpProxys, char* packet, bool newTcpProxy) {
		...
		if((flags & TCPHeader::URG) == TCPHeader::URG){
			...
			printf("[TcpProxy]proxy(%ld%s), packet flags %d program unable to process.\n", id, toString().c_str(), flags);
		}
	}
	...
	void processACKPacket(char* packet) {
		...
		case LAST_ACK:
			...
			break;
		case CLOSED:
			printf("[TcpProxy]proxy(%ld%s) process ack packet state closed, state=%d.\n", id, toString().c_str(), state);
			break;
		case ESTABLISHED:
			processESTABLISHEDACKPacket(packet);
			break;
		default:
			printf("[TcpProxy]proxy(%ld%s) process ack packet state abnormal, state=%d.\n", id, toString().c_str(), state);
			break;
		}
	}
	...
	void processSYNRCVDACKPacket(char* packet) {
		...
		if (tcpHeader.getSeqID() == myAck && tcpHeader.getAckID() == mySeq) {
			...
			printf("[TcpProxy]proxy(%ld%s) initialization synchronous queue number fail.\n", id, toString().c_str());
		}
	}
	...
	void processESTABLISHEDACKPacket(char* packet) {
		...
		if (sequenceNumber == myAck) { 
			//printf("[TcpProxy]proxy(%ld%s) recvive client ack queue number match.\n", id, toString().c_str()); 
			if (dataSize > 0) {
			...
			if (nextSeq > myAck) {
				printf("[TcpProxy]proxy(%ld%s) recvive client ack packet abnormal, seq number %u max %u，数据多包不处理。\n", id, toString().c_str(), nextSeq, myAck);
			} else if(nextSeq > myAck){
				printf("[TcpProxy]proxy(%ld%s) recvive client ack packet abnormal, seq number %u min %u，数据重传不处理。\n", id, toString().c_str(), nextSeq, myAck);
			}
		} else {
			printf("[TcpProxy]proxy(%ld%s) recvive client ack packet abnormal, seq number %u max %u，数据漏包不处理。\n", id, toString().c_str(), sequenceNumber, myAck);
		}
	}
	
	void processRSTPacket(char* packet) {
		printf("[TcpProxy]proxy(%ld%s) recvive client rst packet, closeing.\n", id, toString().c_str()); 
		disConnectClient();
	}
	...
	void processFINPacket(char* packet) {
		// printf("[TcpProxy]proxy(%ld%s) start closeing by client, state=%d.\n", id, toString().c_str(), state); 
		IPHeader ipHeader = IPHeader(packet, 0);
		TCPHeader tcpHeader = TCPHeader(packet, ipHeader.getHeaderLength());
		updateTCPBuffer(headerBytes, TCPHeader::ACK, mySeq, myAck, 0);
		sendToClient(headerBytes, TcpProxy::HEADER_SIZE);
	...
	void processLASTACKPacket(char* packet) {
		...
		if (seq == mySeq && ack == myAck) {
			state = CLOSED;
			// printf("[TcpProxy]proxy(%ld%s) recvive LAST_ACK success, close suceess，seq %u:%u, ack %u:%u.\n", id, toString().c_str(), seq, mySeq, ack, myAck);
		} else {
			printf("[TcpProxy]proxy(%ld%s) recvive LAST_ACK fail, close fail, queue number mismatched, seq %u:%u, ack %u:%u.\n", id, toString().c_str(), seq, mySeq, ack, myAck);
		}
	}
	
	void disConnectClient() {
		// printf("[TcpProxy]proxy(%ld%s) start closeing by server, state=%d.\n", id, toString().c_str(), state); 
		close();
		...
		if(!connected){
			if(cachePackets.size() > 0){
				...
				if(res == 0){
					...
					printf("[TcpProxy]proxy(%ld%s) forward data to server fail, total %d bytes, success send %d bytes.\n", id, toString().c_str(), cachePackets[0].size, res);
					disConnectClient();
					return true;
				}
			}
			if((std::time(NULL) - createTime) > Config::TCP_CONNECT_TIMEOUT){
				printf("[TcpProxy]proxy(%ld%s) connection initialization timeout with server，socket errorno %d.\n", id, toString().c_str(), errno);
				disConnectClient();
				return true;
			}
			return false;
